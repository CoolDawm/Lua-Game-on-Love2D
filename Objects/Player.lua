...
function Player:new(area, x, y, opts)
    ...
    attackst={'Neutral', 'Double' ,'Triple' ,'Rapid' ,'Back','Side',}
...
function Player:update(dt)
    ...
    self.boost = math.min(self.boost + 15*dt, self.max_boost)
    -- Collision
    ...
    self.hp = math.min(self.hp + 50*dt, self.max_hp)
    ...
    if self.boosting then self.trail_color = {76, 195, 217} end
    ...
    if self.collider:enter('Collectable') then
        ...
        elseif object:is(Boost) then
            object:die()
            self:addBoost(15)
        elseif object:is(AttackV) then
            object:die()
            self:setAttack(attackst[love.math.random(1, #attackst)])
        end
    end
    if self.collider:enter('Enemy') then
        ...
        if object:is(EnemyProjectile) then 
            self:hit(10) 
        end
    end
end
...
function Player:draw()
    ...
    for _, vertice_group in ipairs(self.polygons) do
        local points = M.map(vertice_group, function(v, k) if k % 2 == 1 then return self.x + v + random(-1, 1) else return self.y + v + random(-1, 1) end end)
...
function Player:shoot()
    ...
    if self.attack == 'Neutral' then
        ...
        local instance = shoot_sound:play()
...
function Player:die()
    ...
    for i = 1, love.math.random(8, 12) do 
    	self.area:addGameObject('ExplodeParticles', self.x, self.y) 
  	end
    current_room:finish()
end
...
function Player:hit(damage)
    ...
    damage = damage or 10
    for i = 1, love.math.random(4, 8) do self.area:addGameObject('ExplodeParticles', self.x, self.y) end
