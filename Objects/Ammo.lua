...
function Ammo:die()
    self.dead = true
    for i = 1, love.math.random(4, 8) do self.area:addGameObject('ExplodeParticles', self.x, self.y, {s = 3, color = ammo_color}) end
    self.area:addGameObject('AmmoEffect', self.x, self.y, {color = ammo_color, w = self.w, h = self.h})
    self.area:addGameObject('InfoText', self.x, self.y, {text = '+AMMO', color = {0,1,0}})
end
